"""Signals to create gomoku record and profile objects and to update player stats"""
...
def create_gomoku_record_object(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if instance.status == "file":
            ...
            payload = extract_data_from_game_record_file(url=instance.url)
        models.Player.objects.get_or_create(nickname=payload["white"])
        models.Player.objects.get_or_create(nickname=payload["black"])
...
def update_player_stats(sender, instance, created, **kwargs):
    ...
    if created:
        black_player = models.Player.objects.get(nickname=instance.black_player)
        white_player = models.Player.objects.get(nickname=instance.white_player)
...
def create_profile(sender, instance, created, **kwargs):
    ...
    if created:
        models.UserProfile.objects.create(user=instance, slug=instance.username)
